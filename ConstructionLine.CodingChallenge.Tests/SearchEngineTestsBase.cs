        ...
        protected static void AssertSizeCounts(List<Shirt> shirts, SearchOptions searchOptions, List<SizeCount> sizeCounts)
        {
            ...
            foreach (var size in Size.All)
            {
                ...
                var expectedSizeCount = shirts.Count(shirt =>
                    (!searchOptions.Sizes.Any() || searchOptions.Sizes.Contains(size)) && shirt.Size.Id == size.Id &&
                    (!searchOptions.Colors.Any() ||
                     searchOptions.Colors.Select(color => color.Id).Contains(shirt.Color.Id)));
        ...
        protected static void AssertColorCounts(List<Shirt> shirts, SearchOptions searchOptions, List<ColorCount> colorCounts)
        {
            ...
            foreach (var color in Color.All)
            {
                ...
                var expectedColorCount = shirts.Count(shirt =>
                    ...
                    shirt.Color.Id == color.Id && (!searchOptions.Sizes.Any() ||
                                                   searchOptions.Sizes.Select(size => size.Id)
                                                       .Contains(shirt.Size.Id)));
